...
import { getResumableBranches, type ResumableBranchCandidate } from '../utils/resumeBranches.js';
...
export function useInputHandling(params: UseInputHandlingParams) {
  ...
  const queueLayoutRefresh = () => {
    ...
    layoutRefreshDebounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        setStatusMessage(
          `Setting saved but layout refresh failed: ${error?.message || String(error)}`,
        );
  ...
  const openFileBrowserInWorktree = async (selectedPane: DmuxPane) => {
    ...
    const existingBrowserPane = panes.find(
      (pane) => pane.browserPath === selectedPane.worktreePath && !pane.hidden,
    );
  ...
  const handleAddProjectToSidebar = async () => {
    ...
    try {
      ...
      const { createEmptyGitProject, inspectProjectCreationTarget } =
      ...
      if (target.state !== 'missing' && target.state !== 'empty_directory') {
        const message =
          target.state === 'directory_not_empty'
  ...
  const togglePaneVisibility = async (selectedPane: DmuxPane) => {
    ...
    try {
      ...
      await savePanes(
        panes.map((pane) =>
          pane.id === selectedPane.id ? { ...pane, hidden: !selectedPane.hidden } : pane,
        ),
      );
  ...
  const toggleOtherPanesVisibility = async (selectedPane: DmuxPane) => {
    ...
    const targetPanes = panes.filter(
      (pane) =>
        pane.id !== selectedPane.id && (action === 'hide-others' ? !pane.hidden : pane.hidden),
    );
    ...
    try {
      ...
      await savePanes(
        panes.map((pane) => (targetPaneIds.has(pane.id) ? { ...pane, hidden } : pane)),
      );
      await refreshPaneLayout();
      await loadPanes();
  ...
  const toggleProjectPanesVisibility = async (
    ...
    const panesToHide = action === 'focus-project' ? otherPanes.filter((pane) => !pane.hidden) : [];

    try {
      setIsCreatingPane(true);
      setStatusMessage(
        action === 'focus-project' ? `Showing ${projectName} panes...` : 'Showing all panes...',
      );
  ...
  const openPaneMenu = async (pane: DmuxPane, options: { anchorToPane?: boolean } = {}) => {
    const actionId = await popupManager.launchKebabMenuPopup(pane, panes, options);
    if (!actionId) {
      return;
    }
  ...
  const reopenClosedWorktreesInProject = async (targetProjectRoot: string) => {
    ...
    const resumableBranches = await trackProjectActivity(
      async () =>
        getResumableBranches(targetProjectRoot, activeSlugs, {
          includeRemoteBranches: false,
        }),
      targetProjectRoot,
    );
  ...
  useEffect(() => {
    ...
    const queueDrain = () => {
      remoteDrainRef.current = remoteDrainRef.current
        ...
        .catch((error: any) => {
          setStatusMessage(
            `Failed to process remote pane action: ${error?.message || String(error)}`,
          );
          setTimeout(() => setStatusMessage(''), STATUS_MESSAGE_DURATION_LONG);
        });
  ...
  useInput(async (input: string, key: any) => {
    ...
    if (selectedIndex < panes.length && ['a', 'b', 'f', 'A', 'm'].includes(input)) {
      ...
      if (result) {
        try {
          ...
          if (savedCount > 0) {
            const statusMessage =
              savedCount === 1 ? `Setting saved (${lastScope})` : `${savedCount} settings saved`;
      ...
      const demos = [
        ...
        {
          msg: 'This is a longer informational message that will wrap to multiple lines if needed to demonstrate how toasts handle longer content',
      ...
      return;
    } else if (!isLoading && (input === 'p' || input === 'N')) {
      ...
      return;
    } else if (selectedIndex < panes.length && (input === 'j' || input === 'x')) {
      await executePaneShortcut(input as RemotePaneActionShortcut, panes[selectedIndex]);
      return;
