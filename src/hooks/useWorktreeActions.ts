...
export default function useWorktreeActions({
    ...
    async (pane: DmuxPane) => {
      try {
        ...
        if (pane.testWindowId) {
          try {
            await tmuxService.killWindow(pane.testWindowId);
          } catch {}
        }
        if (pane.devWindowId) {
          try {
            await tmuxService.killWindow(pane.devWindowId);
          } catch {}
        }
    ...
    async (pane: DmuxPane) => {
      ...
      try {
        ...
        if (statusOutput.trim()) {
          ...
          execSync(`git -C "${pane.worktreePath}" commit -m 'chore: worktree changes'`, {
            stdio: 'pipe',
          });
        }
  ...
  const mergeAndPrune = useCallback(
    async (pane: DmuxPane) => {
      if (!pane.worktreePath) {
        setStatusMessage('No worktree to merge');
        setTimeout(() => setStatusMessage(''), 2000);
  ...
  const handleCloseOption = useCallback(
    async (option: number, pane: DmuxPane) => {
      switch (option) {
        case 0:
