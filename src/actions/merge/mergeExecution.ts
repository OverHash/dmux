...
export async function executeMerge(
          ...
          onSelect: async (optionId: string) => {
            ...
            if (optionId === 'ai_merge') {
              ...
              const { createConflictResolutionPaneForMerge } =
                await import('./conflictResolution.js');
  ...
  console.error(`[mergeExecution] Pane to close: ${pane.id} (slug: ${pane.slug})`);

  const { closePane } = await import('../implementations/closeAction.js');
  const closeResult = await closePane(pane, context);
  console.error(`[mergeExecution] closePane returned result type: ${closeResult.type}`);
  return closeResult;
}
