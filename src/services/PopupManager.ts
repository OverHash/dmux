...
function isMergeUncommittedChoiceData(data: unknown): data is MergeUncommittedChoiceData {
  ...
  if (typeof candidate.targetBranch !== 'string' || candidate.targetBranch.length === 0) {
    return false;
  }
  if (
...
export class PopupManager {
    ...
    try {
      const popupHandle = await this.trackProjectActivity(async () => {
        ...
        if (options.positioning === 'large') {
          ...
          positioning = POPUP_POSITIONING.centeredWithSidebar(this.config.sidebarWidth);
        } else if (options.positioning === 'pane' && options.targetPaneId) {
          const tmuxService = TmuxService.getInstance();
          const [dims, panePositions] = await Promise.all([
            tmuxService.getAllDimensions(),
            tmuxService.getPanePositions(),
          ]);
          const targetPane = panePositions.find((pane) => pane.paneId === options.targetPaneId);
        ...
        const popupHandle = launchNodePopupNonBlocking<T>(popupScriptPath, args, popupOptions);
  ...
  async launchHooksPopup(onEditHooks: () => Promise<void>, projectRoot?: string): Promise<void> {
    ...
    try {
      ...
      const hooks = allHookTypes.map((hookName) => ({
        name: hookName,
        active: hasHook(hooksProjectRoot || process.cwd(), hookName as any),
      }));
      ...
      if (result.success) {
        ...
        if (typeof data.key === 'string' && (data.scope === 'global' || data.scope === 'project')) {
          if (pendingUpdates.length > 0) {
            return {
              updates: [...pendingUpdates, { key: data.key, value: data.value, scope: data.scope }],
            };
          }
  ...
  async launchEnabledAgentsPopup(projectRoot?: string): Promise<{
    key: 'enabledAgents';
    value: AgentName[];
  ...
  async launchNotificationSoundsPopup(projectRoot?: string): Promise<{
    ...
    try {
      const settings = this.config.settingsManager.getSettings();
      const configuredEnabled = resolveNotificationSoundsSelection(
  ...
  async launchChoicePopup(
    ...
    try {
      ...
      const calculatedHeight = Math.max(12, Math.min(maxHeight, messageLines + optionLines + 6));
  ...
  async launchProgressPopup(
    ...
    try {
      ...
      const titleText = type === 'success' ? '✓ Success' : type === 'error' ? '✗ Error' : 'ℹ Info';
