...
const REMOTE_MENU_TRIGGER_BINDINGS = [{ key: 'M-M', noPrefix: true }] as const;

const LEGACY_REMOTE_TRIGGER_BINDINGS = [{ key: 'M-D', noPrefix: true }] as const;
...
function buildRunRemotePaneActionCommand(shortcut: RemotePaneActionShortcut): string {
...
function buildSafeTmuxCommand(command: string): string {
  return `run-shell "${escapeForDoubleQuotes(`tmux ${command} >/dev/null 2>&1 || true`)}"`;
}

export function isRemotePaneActionShortcut(value: string): value is RemotePaneActionShortcut {
  return REMOTE_PANE_ACTION_SHORTCUTS.includes(value as RemotePaneActionShortcut);
}
...
export function getTmuxSessionOption(sessionName: string, optionName: string): string | null {
  try {
...
export async function clearRemotePaneActions(sessionName: string, homeDir?: string): Promise<void> {
  const queuePath = getRemotePaneActionQueuePath(sessionName, homeDir);
...
export function buildRemotePaneActionCleanupCommands(): string[] {
  const commands = [...REMOTE_MENU_TRIGGER_BINDINGS, ...LEGACY_REMOTE_TRIGGER_BINDINGS].map(
    ({ key, noPrefix }) =>
      buildSafeTmuxCommand(noPrefix ? `unbind-key -n ${key}` : `unbind-key ${key}`),
  );

  commands.push(
    buildSafeTmuxCommand(`unbind-key -T ${DMUX_REMOTE_PANE_ACTION_TABLE} Escape`),
    buildSafeTmuxCommand(`unbind-key -T ${DMUX_REMOTE_PANE_ACTION_TABLE} C-c`),
    buildSafeTmuxCommand(`unbind-key -T ${DMUX_REMOTE_PANE_ACTION_TABLE} Any`),
  );

  for (const shortcut of REMOTE_PANE_ACTION_SHORTCUTS) {
    commands.push(
      buildSafeTmuxCommand(`unbind-key -T ${DMUX_REMOTE_PANE_ACTION_TABLE} ${shortcut}`),
    );
  }
