        ...
        if (!allTmuxPaneIds.includes(id)) {
          if (!suppressLogs) {
            LogService.getInstance().debug(
              `Skipping stale pane ID ${id} (no longer exists in tmux)`,
              'Layout',
            );
          }
          return false;
    ...
    try {
      ...
      const validFinalPanes = finalContentPanes.filter((id) => {
        if (!currentPaneIds.includes(id)) {
          if (!suppressLogs) {
            LogService.getInstance().debug(
              `Pane ${id} disappeared before layout application`,
              'Layout',
            );
          }
          return false;
      ...
      if (!suppressLogs) {
        LogService.getInstance().debug(
          `Final validation failed: ${validationError}, attempting layout anyway`,
          'Layout',
        );
      }
      layoutApplier.applyPaneLayout(
