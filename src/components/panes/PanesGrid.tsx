...
import { buildProjectActionLayout, type ProjectActionItem } from '../../utils/projectActions.js';
...
const PanesGrid: React.FC<PanesGridProps> = memo(
  ({
    ...
    const actionLayout = useMemo(
      () =>
        buildProjectActionLayout(panes, sidebarProjects, fallbackProjectRoot, fallbackProjectName),
        ...
        {paneGroups.map((group, groupIndex) => (
          <Box key={group.projectRoot} flexDirection="column">
            {(() => {
              ...
              const remaining = Math.max(0, HEADER_WIDTH - stringWidth(nameSection) - spinnerWidth);
            ...
            {group.panes.map((entry) => {
              ...
              const isDevSource = isActiveDevSourcePath(pane.worktreePath, activeDevSourcePath);
