...
const EnabledAgentsPopupApp: React.FC<EnabledAgentsPopupProps> = ({ resultFile, data }) => {
  const [mode, setMode] = useState<'list' | 'scope'>('list');
  const [selectedIndex, setSelectedIndex] = useState(0);
  const [scopeIndex, setScopeIndex] = useState(0);
  const [enabled, setEnabled] = useState<Set<AgentName>>(() => new Set(data.enabledAgents));
  const [installStatus, setInstallStatus] = useState<
    Partial<Record<AgentName, 'loading' | 'installed' | 'notInstalled'>>
  >(
    () =>
      Object.fromEntries(data.agents.map((agent) => [agent.id, 'loading'])) as Partial<
        Record<AgentName, 'loading' | 'installed' | 'notInstalled'>
      >,
  );
  const { exit } = useApp();
  ...
  const orderedEnabledAgents = useMemo(() => {
    return data.agents.map((agent) => agent.id).filter((agentId) => enabled.has(agentId));
  }, [data.agents, enabled]);
      ...
      >
        {mode === 'list' && (
          <Box flexDirection="column">
            <Box marginBottom={1}>
              <Text dimColor>Enabled agents appear in the new-pane selection popup.</Text>
              <Text dimColor>Install status is checked now for each agent.</Text>
