...
import { expandTilde, parsePathInput, scanDirectories } from '../../utils/dirScanner.js';
...
export const ProjectSelectApp: React.FC<ProjectSelectProps> = ({ resultFile, defaultValue }) => {
  ...
  useInput((input, key) => {
    ...
    if (key.downArrow) {
      if (directories.length > 0) {
        setSelectedDirIndex((prev) => Math.min(directories.length - 1, prev + 1));
      }
      return;
    }
    if (key.upArrow) {
  ...
  return (
    ...
    >
      <PopupContainer footer={footer}>
        ...
        <DirectoryList directories={directories} selectedIndex={selectedDirIndex} maxVisible={10} />

        {shouldShowCreateProjectHint && (
          <Box marginTop={1}>
            <Text color="blue">Hit Enter to create a new project at this location.</Text>
          </Box>
        )}
      </PopupContainer>
    </PopupWrapper>
...
function main() {
  ...
  render(<ProjectSelectApp resultFile={resultFile} defaultValue={defaultValue} />);
}
