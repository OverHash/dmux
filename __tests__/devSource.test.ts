...
import { isActiveDevSourcePath, resolveNextDevSourcePath } from '../src/utils/devSource.js';

describe('devSource utils', () => {
  it('detects active source path match', () => {
    expect(
      isActiveDevSourcePath('/repo/.dmux/worktrees/feature-a', '/repo/.dmux/worktrees/feature-a'),
    ).toBe(true);
  });

  it('returns false when pane has no worktree path', () => {
    expect(isActiveDevSourcePath(undefined, '/repo/.dmux/worktrees/feature-a')).toBe(false);
  });
