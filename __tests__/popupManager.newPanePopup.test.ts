...
describe('PopupManager launchNewPanePopup', () => {
  it('normalizes structured popup payloads and uses project-specific settings', async () => {
  ...
  it('keeps empty prompts when git overrides are provided', async () => {
    const manager = createPopupManager({ promptForGitOptionsOnCreate: true }) as any;
    manager.checkPopupSupport = vi.fn(() => true);
    manager.launchPopup = vi.fn().mockResolvedValue({
      success: true,
      data: { prompt: '', baseBranch: ' main ', branchName: ' feat/LIN-9 ' },
    });
