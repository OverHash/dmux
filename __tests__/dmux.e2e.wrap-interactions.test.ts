...
function detectRunner(): { cmd: string; label: string } | null {
  ...
  if (hasCmd('tsx'))
    return { cmd: `tsx "${path.join(process.cwd(), 'src', 'index.ts')}"`, label: 'tsx-src' };
  return null;
}
...
function getInputLines(captured: string): string[] {
  ...
  for (const l of lines) {
    if (l.startsWith('> ') || l.startsWith('  '))
      inputLines.push(l.replace(/^>\s/, '').replace(/^\s{2}/, ''));
  }
  return inputLines;
}
...
describe.sequential('dmux e2e: input wrapping interactions', () => {
  it.runIf(canRun)(
    'wraps full word at the moment overflow occurs (on screen)',
    async () => {
      ...
      try {
        try {
          execSync(`tmux -L ${server} kill-session -t ${session}`, { stdio: 'pipe' });
        } catch {}
        try {
          execSync(`tmux -L ${server} kill-server`, { stdio: 'pipe' });
        } catch {}

        execSync(`tmux -L ${server} -f /dev/null new-session -d -s ${session} -n main bash`, {
          stdio: 'pipe',
        });
        execSync(`tmux -L ${server} send-keys -t ${session}:0.0 'export HOME="${tmpHome}"' Enter`, {
          stdio: 'pipe',
        });
        ...
        execSync(`tmux -L ${server} send-keys -t ${session}:0.0 '${runner!.cmd}' Enter`, {
          stdio: 'pipe',
        });
        ...
        execSync(`tmux -L ${server} send-keys -t ${session}:0.0 '${text[wrapTriggerIndex]}'`, {
          stdio: 'pipe',
        });
        await sleep(80);
